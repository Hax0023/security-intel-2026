...
## Metadata
- **Source:** HackerOne
- **Report:** 3580511 | https://hackerone.com/reports/3580511
...
ActiveStorage's `DiskService#path_for` does not validate or sanitize blob keys before constructing file paths. Combined with the Hash attachable interface — which passes user-supplied `key:` values directly to `Blob.build_after_unfurling` without filtering — an attacker who can influence the Hash passed to `.att
