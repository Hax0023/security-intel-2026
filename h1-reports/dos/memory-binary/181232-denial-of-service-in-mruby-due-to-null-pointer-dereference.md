# Denial of Service in mruby due to null pointer dereference in ary_modify

## Metadata
- **Source:** HackerOne
- **Report:** 181232 | https://hackerone.com/reports/181232
- **Submitted:** 2016-11-10
- **Reporter:** haquaman
- **Program:** mruby
- **Bounty:** Not specified
- **Severity:** high
- **Vuln:** Null Pointer Dereference, Denial of Service, Segmentation Fault
- **CVEs:** None
- **Category:** memory-binary

## Summary
A null pointer dereference vulnerability exists in mruby's array handling code that can be triggered by specific Ruby code patterns combining splat operators with case statements and redo blocks. The crash occurs when a null RArray struct is passed to mrb_ary_push, which attempts to dereference it without null checking, causing a segmentation fault.

## Attack scenario
1. Attacker crafts a malicious Ruby script containing the specific pattern: splat operator on a string, case statement, and redo with block parameter
2. Attacker provides the script to a mruby interpreter or application embedding mruby (such as mruby-engine)
3. During execution, the parser/compiler generates bytecode that leads to a null RArray being passed to mrb_ary_push
4. The mrb_ary_push function calls ary_modify without null checking the RArray pointer
5. ary_modify attempts to dereference the null pointer, accessing memory at address 0x0
6. The process segfaults (SIGSEGV), denying service to legitimate users

## Root cause
The vulnerable code path occurs due to improper handling of the redo statement within a case block with block parameters. The code generation likely produces an uninitialized or null array reference that gets passed to mrb_ary_push. The function lacks defensive null pointer checking before attempting to access struct members.

## Attacker mindset
An attacker could exploit this for denial of service attacks against mruby-based services. The minimal, obfuscated nature of the crash code suggests the finder reversed engineered the issue. The attacker mindset focuses on finding edge cases in statement combinations (splat + case + redo) that trigger uninitialized state.

## Defensive takeaways
- Implement null pointer checks in functions that dereference pointers before use, especially in array manipulation functions
- Add input validation and state verification at VM instruction boundaries
- Implement comprehensive fuzzing of language feature combinations to catch edge cases
- Use static analysis tools to detect potential null dereference issues
- Add assertions during debug builds to catch null dereferences early
- Sanitize code generation for complex control flow patterns (redo with blocks)
- Consider using safe pointer alternatives or wrapper types that enforce null checks

## Variant hunting
Similar null pointer dereference vulnerabilities likely exist in: (1) other collection manipulation functions (hash, string operations), (2) other control flow statements combined with redo/retry, (3) block parameter handling with yield operations, (4) splat operator expansion in different contexts, (5) edge cases in case statement with various block constructs
...
## Notes
The vulnerability requires a specific code pattern suggesting it was found through fuzzing or manual code review of language feature interactions. The crash occurs in mrb_ary_ptr->ary_modify->accessing struct members at address 0x0, with rsi=0x0 (null pointer). The issue also propagates to mruby-engine, affecting Ruby applications using the mruby sandbox. The proposed patch is minimal (null check + early return) but the FIXME comment indicates uncertainty about proper error handling semantics.
