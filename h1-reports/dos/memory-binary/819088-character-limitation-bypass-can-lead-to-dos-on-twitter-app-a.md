# Character Limitation Bypass Leading to DoS and 500 Internal Server Error on Twitter Moments

## Metadata
- **Source:** HackerOne
- **Report:** 819088 | https://hackerone.com/reports/819088
- **Submitted:** 2020-03-14
- **Reporter:** exit_n0de
- **Program:** Twitter
- **Bounty:** Not specified in report
- **Severity:** High
- **Vuln:** Input Validation Bypass, Denial of Service (DoS), Resource Exhaustion, Improper Error Handling
- **CVEs:** None
- **Category:** memory-binary

## Summary
A character limitation bypass on Twitter's Moments creation feature allows attackers to submit excessively large payloads that bypass client-side validation. This causes 500 Internal Server Error responses on the backend and triggers severe resource consumption on the Android app, potentially crashing it when victims access malicious Moment links.

## Attack scenario
1. Attacker creates a new Moment via the Twitter web interface at https://twitter.com/{username}/moments
2. Attacker intercepts the moment creation request using a proxy tool like Burp Suite
3. Attacker modifies the title or description parameter with extremely large payloads (1.9MB+ of characters), bypassing the documented 60/250 character limits
4. Attacker sends the modified request to the backend API, triggering a 500 Internal Server Error due to improper input validation and resource handling
5. Attacker shares the malicious Moment link via tweet or direct message to other users
6. When victims click the link and access the Moment in the Android app, the app hangs, consumes excessive resources, and may crash when attempting to share or interact with the content

## Root cause
Insufficient input validation on the backend API for Moment title and description fields. Character limits are enforced only on the client-side UI, allowing attackers to bypass them via direct API requests. The server lacks proper bounds checking and graceful error handling for oversized payloads, resulting in 500 errors and resource exhaustion rather than immediate rejection with appropriate error codes.

## Attacker mindset
An attacker with this knowledge could weaponize malicious Moment links to cause widespread DoS attacks across the Twitter platform by distributing links that crash the mobile app. The ability to disguise attacks as legitimate Twitter URLs increases success rates. The attacker demonstrates curiosity-driven security research, progressively testing boundaries to understand the full impact chain from backend errors to mobile app crashes.

## Defensive takeaways
- Implement strict server-side input validation with hard limits on all user-supplied fields, independent of client-side constraints
- Enforce character/size limits at the API handler level before any processing or database operations
- Return appropriate HTTP status codes (400 Bad Request) for oversized payloads instead of allowing 500 errors; implement request size limits at the web server layer
- Add comprehensive rate limiting and per-user quotas for resource-intensive operations like Moment creation
- Implement proper error handling and logging to detect abuse patterns (multiple large payload submissions from same user)
- Test mobile app resilience against malformed or oversized data responses to prevent crashes and excessive resource consumption
- Consider implementing content-aware limits (e.g., database field size constraints) as a final defense layer
- Monitor for anomalous Moment access patterns and high error rates that may indicate attack campaigns

## Variant hunting
Similar character limit bypass vulnerabilities likely exist in other Twitter features with client-side validation only: Tweet composition (though more protected), bio/profile descriptions, hashtag titles, custom timeline names, and other user-generated content fields. Mobile apps accepting user-provided links or data should be tested for resource exhaustion via malformed payloads. Search for other endpoints where request size restrictions are handled at web server level (413 errors) but not at API logic level.
...
## Notes
The reporter demonstrated exceptional security research methodology by progressively testing payload sizes and discovering the dual impact (backend 500 errors + mobile app DoS). The 413 Request Entity Too Large error at 1.9MB suggests web server-level size limits exist but don't prevent smaller payloads that still cause backend failures. The Android app's inability to handle large text fields safely is a secondary but severe vulnerability. No iOS testing was completed. The ability to trigger this via share functionality expands attack surface significantly.
