# Cache Poisoning DoS to various TTS assets via X-CF-APP-INSTANCE Header

## Metadata
- **Source:** HackerOne
- **Report:** 728664 | https://hackerone.com/reports/728664
- **Submitted:** 2019-11-04
- **Reporter:** nathand
- **Program:** Technology Transformation Services (TTS) / GSA
- **Bounty:** Not specified
- **Severity:** high
- **Vuln:** Cache Poisoning, Denial of Service, HTTP Response Splitting, Information Disclosure
- **CVEs:** None
- **Category:** uncategorised

## Summary
A cache poisoning vulnerability exists in CloudFoundry-based TTS applications where the X-CF-APP-INSTANCE header can be abused to force HTTP 404 responses that are then cached by upstream CDNs like CloudFront. An attacker can systematically poison cache entries for resources, causing legitimate users to receive 404 errors, effectively denying service to the application.

## Attack scenario
1. Attacker identifies TTS applications running on CloudFoundry behind CDN caches (Cloudfront, Cloudflare)
2. Attacker crafts malicious requests with X-CF-APP-INSTANCE header set to invalid values (e.g., 'xxx:1')
3. CloudFoundry gorouter processes the invalid header and returns HTTP 404 response with cache-friendly headers
4. Upstream CDN caches the 404 response, keying on URL and other request parameters
5. Subsequent legitimate users requesting the same resource receive the cached 404 error
6. Attacker can scale attack across multiple regions using distributed VPS infrastructure to poison caches globally

## Root cause
CloudFoundry's gorouter processes the X-CF-APP-INSTANCE debug header without proper validation and returns cacheable HTTP 404 responses when the header contains invalid values. The upstream CDN caches these responses without distinguishing between legitimate 404s and those caused by malformed headers, allowing attacker-controlled cache poisoning.

## Attacker mindset
An attacker would recognize this as a low-effort, high-impact vulnerability requiring minimal technical sophistication. The ability to systematically DoS government services like analytics.usa.gov and federation.data.gov with distributed cache poisoning represents significant reputational damage. The bonus information disclosure of internal CloudFoundry hostnames adds reconnaissance value.

## Defensive takeaways
- Configure CDN and WAF to strip X-CF-APP-INSTANCE headers from external requests to prevent header injection
- Exclude HTTP 404 responses from caching policy, or implement cache key normalization that ignores debug headers
- Implement rate limiting on requests with invalid X-CF-APP-INSTANCE values at the gorouter level
- Monitor for repeated 404 responses from specific source IPs or regions as indicators of cache poisoning attacks
- Disable or restrict CloudFoundry debug headers in production environments (implement at gorouter layer)
- Log all requests with X-CF-APP-INSTANCE headers separately for security monitoring
- Align caching configuration with login.gov's approach which appears to successfully mitigate this attack

## Variant hunting
Hunt for similar debug/admin headers in other load balancers and reverse proxies (nginx X-Original-URL, Apache X-Forwarded-For misconfigurations, Kubernetes service mesh headers) that could be abused for cache poisoning. Test any header designed for debugging/routing that triggers error responses when malformed. Examine other CloudFoundry deployments and gorouter configurations for similar vulnerabilities.
...
## Notes
Reporter demonstrated good disclosure practices by reporting to Pivotal/CloudFoundry first before TTS. The vulnerability is particularly impactful because: (1) it affects government services, (2) it's trivial to exploit at scale, (3) error logs may not capture poisoning attempts occurring at gorouter level, (4) regional CloudFront caching allows targeted attacks, and (5) internal hostname disclosure reveals infrastructure details. The cache keying behavior appears to use Cookie headers, limiting exposure to unauthenticated users but still significant for public-facing assets.
