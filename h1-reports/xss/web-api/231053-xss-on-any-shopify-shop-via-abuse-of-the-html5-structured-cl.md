# XSS on any Shopify shop via HTML5 structured clone algorithm bypass in postMessage listener

## Metadata
- **Source:** HackerOne
- **Report:** 231053 | https://hackerone.com/reports/231053
- **Submitted:** 2017-05-23
- **Reporter:** bored-engineer
- **Program:** Shopify
- **Bounty:** Not specified in writeup
- **Severity:** high
- **Vuln:** Cross-Site Scripting (XSS), Insufficient Input Validation, Insecure postMessage handling, Object prototype pollution variant
- **CVEs:** None
- **Category:** web-api

## Summary
The /digital_wallets/dialog endpoint contains a postMessage listener without origin validation that processes user-controlled data. Although the application attempts to escape HTML content, attackers can bypass this sanitization by leveraging the HTML5 structured clone algorithm to inject specially crafted objects that circumvent escaping mechanisms, resulting in arbitrary XSS execution.

## Attack scenario
1. Attacker identifies the digital_wallets/dialog endpoint which listens for postMessage events without origin validation
2. Attacker crafts a malicious postMessage containing a clone-able object that exploits the structured clone algorithm's handling of object properties
3. The application receives the message and validates required fields (title, button) which pass the basic checks
4. The payload is processed through the escaping function which fails to properly sanitize the specially-crafted clone-able object due to algorithm quirks
5. The unsanitized content is inserted into the DOM via innerHTML, allowing arbitrary JavaScript execution
6. The XSS payload executes in the context of the Shopify shop, allowing session hijacking, credential theft, or other malicious actions

## Root cause
The application relies on a simple escaping function (u()) to sanitize user input before DOM insertion, but this escaping is ineffective against objects crafted using the HTML5 structured clone algorithm. The postMessage listener accepts messages from any origin (wildcard '*') without validation, and the deserialization of complex objects bypasses string-based sanitization that only addresses string payloads.

## Attacker mindset
An attacker would recognize that standard XSS filters focus on string escaping while missing the semantic meaning of JavaScript objects during serialization/deserialization. By weaponizing the structured clone algorithm (commonly used in Web Workers and postMessage), they can craft objects whose properties are reconstructed in ways that bypass string-level escaping, turning a seemingly well-defended endpoint into an XSS vector.

## Defensive takeaways
- Always validate the origin of postMessage events using event.origin checks, never rely on wildcard acceptance
- Implement origin whitelisting for cross-origin communication
- Do not rely solely on string escaping functions when handling complex serialized objects - validate data types and structure
- Use Content Security Policy (CSP) to prevent inline script execution and mitigate XSS impact
- Prefer textContent over innerHTML when inserting user-controlled data
- Implement strict input validation that rejects unexpected object types or properties before processing
- Test sanitization functions against complex objects, not just string payloads
- Consider using a robust HTML sanitization library rather than custom escaping logic

## Variant hunting
Look for similar postMessage listeners throughout Shopify's codebase that: (1) lack origin validation, (2) use innerHTML with escaped user input, (3) process complex objects without type validation. Search for other endpoints accepting serialized data from untrusted sources. Test structured clone edge cases with Date objects, typed arrays, Maps, Sets, and nested objects with circular references that might preserve dangerous properties through the escaping function.
...
## Notes
This vulnerability demonstrates a sophisticated attack that combines multiple weaknesses: missing origin validation, inadequate object deserialization handling, and flawed sanitization logic. The HTML5 structured clone algorithm is rarely considered in threat models but can be weaponized when applications deserialize complex objects. The fact that basic validation (checking for required string fields) passes while the actual dangerous data is smuggled through object properties shows the importance of deep input validation. The report references minified production code, making analysis harder but the vulnerability remains exploitable.
