...
## Metadata
- **Source:** HackerOne
- **Report:** 398054 | https://hackerone.com/reports/398054
- **Submitted:** 2018-08-22
- **Reporter:** adac95
- **Program:** HackerOne
- **Bounty:** Not specified in report
- **Severity:** medium
- **Vuln:** DOM-based XSS, Postmessage XSS, Insecure deserialization, Open redirect
- **CVEs:** None
- **Category:** web-api

## Summary
The Marketo contact form on hackerone.com contains a DOM-based XSS vulnerability via unsafe handling of postMessage events. An attacker can craft a malicious JSON message with a javascript: URI in the followUpUrl property, causing arbitrary JavaScript execution. The vulnerability is mitigated by Content Security Policy and requires users to visit an attacker-controlled page first.

## Attack scenario
1. Attacker creates a malicious webpage containing JavaScript that sends a crafted postMessage to hackerone.com window
2. The malicious message contains a JSON payload with mktoResponse object and javascript: URI in followUpUrl
3. Victim visits attacker's webpage while hackerone.com is open in another tab/frame
4. The postMessage listener on hackerone.com receives the message without origin validation
5. The onMessage function parses the JSON and calls onResponse with the attacker-controlled data
6. The success function redirects location.href to the javascript: URI, executing arbitrary code in hackerone.com context

## Root cause
Insecure postMessage implementation lacking origin validation and insufficient sanitization of followUpUrl property before using it in location.href assignment. The message listener accepts messages from any origin and directly uses user-supplied data in navigation operations.

## Attacker mindset
An attacker would seek to execute JavaScript in the context of hackerone.com to steal session cookies, capture form data, redirect users to phishing sites, or deface content. The cross-origin nature of postMessage exploitation is attractive because it can be triggered from attacker-controlled frames or windows without direct access to hackerone.com.

## Defensive takeaways
- Always validate the origin of postMessage events using e.origin === 'https://expected-origin.com' before processing
- Never use user-supplied data directly in location.href or location.assign() without proper URL validation
- Implement URL validation to ensure redirects only go to whitelisted domains or relative URLs
- Use URL parsing APIs to validate that followUpUrl is not a javascript: or data: URI before navigation
- Apply Content Security Policy with script-src restrictions as a defense-in-depth measure
- Sanitize and validate all external data before using in security-sensitive contexts
- Consider using a URL validator library that explicitly rejects dangerous URI schemes

## Variant hunting
Search for similar patterns: (1) Other postMessage listeners without origin validation across web applications; (2) Unmarked/unvalidated use of location.href with external data; (3) JSON parsing followed by property access without schema validation; (4) Third-party embeds (Marketo, HubSpot, Zendesk) with unsafe message handlers; (5) Forms that accept redirect/followUp URLs without strict validation; (6) Cross-frame communication in applications without proper CORS/origin checks
...
## Notes
The vulnerability was partially mitigated by HackerOne's CSP policy which blocks javascript: URI redirects. However, CSP alone should not be the primary defense. The report demonstrates good security research methodology by identifying that the vulnerability could still affect users on browsers without CSP support. The requirement for users to visit attacker's site first increases the attack complexity, but sophisticated attackers could embed the payload in legitimate-looking pages or use clickbait tactics. This is a good example of how third-party embedded scripts (Marketo forms2.js) can introduce security issues despite being from reputable vendors.
...
An attacker could be able to execute JavaScript in the context of the www.hackerone.com application, if the victim user makes 
