# Stored XSS in Mermaid Markdown with CSP Bypass via Pipeline Artifacts

## Metadata
- **Source:** HackerOne
- **Report:** 1212822 | https://hackerone.com/reports/1212822
- **Submitted:** 2021-05-30
- **Reporter:** saleemrashid
- **Program:** GitLab
- **Bounty:** $10,000+
- **Severity:** critical
- **Vuln:** Stored XSS, CSP Bypass, Type Coercion Vulnerability, Arbitrary Code Execution
- **CVEs:** None
- **Category:** web-api

## Summary
GitLab's Mermaid diagram renderer contains a stored XSS vulnerability caused by improper type checking when evaluating the htmlLabels configuration option. An attacker can inject malicious HTML/JavaScript by setting htmlLabels to the string 'false' instead of boolean false, bypassing sanitization while still rendering as HTML. This XSS can be escalated to RCE by exploiting a CSP bypass using pipeline artifacts served with auto-detected Content-Type headers.

## Attack scenario
1. Attacker creates or gains access to a GitLab repository
2. Attacker creates a CI/CD pipeline that outputs a JavaScript file as an artifact with .js extension
3. Attacker crafts a Mermaid diagram in README.md with init directive setting flowchart.htmlLabels to string 'false'
4. Attacker embeds an iframe with srcdoc containing a script tag pointing to the pipeline artifact URL
5. When a user views the README.md, the Mermaid diagram renders unsanitized HTML due to type coercion bug
6. The iframe loads the artifact as JavaScript (Content-Type auto-detection bypasses CSP restrictions), executing arbitrary code as the authenticated user

## Root cause
Type coercion vulnerability in Mermaid's configuration handling: the code checks `config.flowchart.htmlLabels === false || config.flowchart.htmlLabels === 'false'` for sanitization decisions, but later uses `if (config.flowchart.htmlLabels)` for rendering decisions. The string 'false' is truthy in JavaScript, causing unsanitized content to be rendered as HTML. Additionally, GitLab's Workhorse serves pipeline artifacts with Content-Type headers auto-detected from file extensions, allowing JavaScript files to bypass CSP nonce restrictions.

## Attacker mindset
An attacker recognized that Mermaid directives could modify configuration options and specifically exploited a type coercion flaw where string 'false' differs from boolean false. They then identified that CSP with nonces could be bypassed by leveraging GitLab's infrastructure feature (pipeline artifacts) to serve JavaScript with a legitimate Content-Type header, turning a UI-level XSS into full account compromise.

## Defensive takeaways
- Use strict type checking (=== and !== only) when evaluating boolean security-critical configuration options; avoid coercing strings to booleans
- Implement deny-lists for user-controllable directives that can modify security-relevant configurations; only allow whitelisted overrides
- Serve user-controlled artifacts with Content-Disposition: attachment and explicit Content-Type: application/octet-stream to prevent browser interpretation
- Consider removing Content-Type auto-detection for untrusted files; force explicit MIME types
- Apply CSP restrictions consistently across all content sources, including internally-served artifacts
- Sanitize all HTML output regardless of configuration state; don't rely on config flags as the sole sanitization trigger
- Test security controls with both boolean and string representations of configuration values

## Variant hunting
Search for other diagram libraries (PlantUML, etc.) with similar directive-based config override mechanisms
Test other boolean-valued security configurations for type coercion vulnerabilities (e.g., allowScripts, sanitize, safe)
Identify other GitLab features serving user content with auto-detected Content-Type headers
Look for CSP bypasses using other auto-typed content sources (SVG files, JSON endpoints, etc.)
Test whether other string-to-boolean coercions exist in Mermaid or similar libraries
Check if pipeline artifacts can be served from different domains or with different headers to further bypass restrictions
...
## Notes
This is a sophisticated multi-stage exploit combining three vulnerabilities: (1) type coercion in Mermaid config validation, (2) CSP bypass via artifact content-type handling, and (3) stored XSS. The use of iframe srcdoc with external script source is clever because it doesn't trigger CSP violations at the iframe level. GitLab's response likely involved fixing type checking in Mermaid config, implementing stricter Content-Type handling for artifacts, and possibly updating CSP policies. This demonstrates the importance of defense-in-depth when both application logic and infrastructure features are involved in security boundaries.
