...
## Metadata
- **Source:** HackerOne
- **Report:** 172137 | https://hackerone.com/reports/172137
- **Submitted:** 2016-09-26
- **Reporter:** arneswinnen
- **Program:** Ubiquiti Networks
- **Bounty:** Not specified in excerpt
- **Severity:** critical
- **Vuln:** Subdomain Takeover, Dangling DNS CNAME, Session Cookie Leakage, Insecure Cookie Scope, Authentication Bypass
- **CVEs:** None
- **Category:** web-api

## Summary
A chain of two vulnerabilities enables full SSO authentication bypass: the subdomain ping.ubnt.com pointed to an unregistered AWS CloudFront distribution allowing takeover, and SSO session cookies were shared across all *.ubnt.com subdomains via overly-broad domain attribute. An attacker could hijack authenticated users' sessions by causing them to make requests to the compromised subdomain.

## Attack scenario
1. Attacker discovers ping.ubnt.com is a CNAME pointing to unregistered/unclaimed AWS CloudFront distribution
2. Attacker registers or claims the CloudFront distribution and deploys malicious webserver content
3. Attacker obtains valid SSL certificate for ping.ubnt.com (via Let's Encrypt automated verification using subdomain control)
4. Attacker tricks authenticated victim into making request to https://ping.ubnt.com via embedded image in email, iframe, or community post
5. Victim's browser automatically sends UBIC_AUTH session cookie (domain=.ubnt.com, secure flag set) to attacker's server
6. Attacker uses stolen UBIC_AUTH session cookie to impersonate victim on all *.ubnt.com services and connected devices via centralized SSO

## Root cause
Two compounding misconfigurations: (1) dangling CNAME DNS record pointing to unregistered/unclaimed external CDN resource enabling subdomain takeover, and (2) overly permissive cookie domain attribute (.ubnt.com) causing session cookies to be shared with all subdomains without proper origin validation or cookie scoping

## Attacker mindset
This chain attack demonstrates sophisticated understanding of DNS/CDN infrastructure, cookie mechanics, and browser security model. Attacker recognized that combining subdomain takeover with cookie leakage bypasses SSO entirely. Attack is stealthy (silent requests via IMG tags), scalable (waterhole attack on community.ubnt.com), and high-impact (centralized SSO affects all connected services). Attacker showed restraint by not obtaining actual SSL certificate.

## Defensive takeaways
- Audit all DNS records for dangling CNAME/NS/MX pointing to external services (CDN, cloud providers); ensure all referenced external resources are actively registered and controlled
- Implement subdomain enumeration and monitoring to detect unclaimed/deprecated subdomains
- Scope session cookies narrowly: use specific subdomain (sso.ubnt.com) instead of wildcard (.ubnt.com) unless business logic requires sharing
- Implement SameSite cookie attribute (Strict or Lax) to prevent cross-site cookie leakage
- Use Content-Security-Policy headers to limit resource loading from unintended origins
- Implement request origin/referer validation on sensitive endpoints
- Monitor for cross-subdomain requests and implement CORS restrictions
- Use HSTS with includeSubDomains to enforce HTTPS across all subdomains
- Implement certificate transparency monitoring to detect unauthorized certificates for your domain

## Variant hunting
Look for: (1) Other *.ubnt.com subdomains with dangling DNS records (other CDN providers, hosting services, third-party integrations); (2) Other Ubiquiti properties sharing cookies broadly; (3) Similar overly-broad cookie domain attributes in other web properties; (4) CloudFront distributions registered to competitors/lapsed accounts in domain takeover registries; (5) Subdomains pointing to deprecated/removed Azure, Akamai, CloudFlare resources; (6) OAuth/SAML integration endpoints that may inherit compromised SSO session
...
## Notes
This is a critical authentication chain exploiting infrastructure misconfiguration + cookie architecture flaw. The researcher demonstrated maturity by not completing the attack (obtaining actual SSL cert) but providing clear PoC path. The mass exploitation scenario via community.ubnt.com waterhole is particularly dangerous as it requires only that victims read a forum post while authenticated. The centralized SSO design amplifies impact - single compromise affects all connected services/devices. Fix requires both immediate remediation (add to PSCAN, re-register CloudFront) and architectural changes (cookie scoping, origin validation).
...
In a real attack scenario, an attacker would probably want to store the received cookie locally and use a cookie addon for his/her specific browser, to inject the session cookie value in the browser more persi
