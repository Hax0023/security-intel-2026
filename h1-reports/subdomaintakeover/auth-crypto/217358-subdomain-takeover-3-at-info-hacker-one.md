# Subdomain Takeover via Parameter Injection at Unbounce Pages

## Metadata
- **Source:** HackerOne
- **Report:** 217358 | https://hackerone.com/reports/217358
- **Submitted:** 2017-03-30
- **Reporter:** ak1t4
- **Program:** HackerOne
- **Bounty:** Not specified in report
- **Severity:** high
- **Vuln:** Subdomain Takeover, Parameter Injection, Improper Input Validation, Mass Assignment Vulnerability
- **CVEs:** None
- **Category:** auth-crypto

## Summary
A mass assignment vulnerability in Unbounce Pages allows authenticated attackers to inject arbitrary domains via the page[url] parameter during page creation/editing, enabling subdomain takeover of branded domains like info.hacker.one. This bypasses a previous fix (report #209004) by targeting a newly exposed parameter endpoint.

## Attack scenario
1. Attacker authenticates to Unbounce application and creates or accesses an existing page
2. Attacker navigates to the 'EDIT NAME' functionality and initiates a page update request
3. Attacker intercepts the HTTP PUT request with a proxy tool and identifies the vulnerable page[url] parameter
4. Attacker modifies the request body to inject a target domain (e.g., page%5Burl%5D=info.hacker.one/takeover-bypass)
5. Server accepts and processes the injected parameter without validation, binding it to the page object
6. Attacker hosts content on the claimed subdomain and uses it for phishing, credential theft, or malware distribution

## Root cause
The application implements mass assignment vulnerability by accepting arbitrary page object attributes from user input without whitelist validation. The parameter injection through page[url] bypasses domain ownership verification, allowing users to claim subdomains they don't control. Prior security fix did not comprehensively address parameter binding logic.

## Attacker mindset
An attacker recognizes the incomplete nature of the previous security patch and systematically tests alternative parameters that affect domain binding. They leverage the fact that authenticated users have access to create pages and exploit the trust in the application's internal parameter handling. The attacker understands that brand domains carry trust and can be weaponized for phishing or credential harvesting attacks.

## Defensive takeaways
- Implement strict whitelist-based parameter binding; only allow explicitly defined attributes for mass assignment
- Add domain ownership verification before allowing subdomain claims (DNS CNAME validation, file upload confirmation)
- Validate that domain modification requests include proof of ownership or are restricted to user-controlled domains
- Implement comprehensive input validation and sanitization for all domain-related parameters
- Add audit logging for domain claim/modification operations to detect suspicious activity
- Use security testing frameworks to verify all object attributes are protected against injection
- Implement rate limiting on domain claims and alert on multiple failed ownership verifications

## Variant hunting
Test similar mass assignment vulnerabilities in other page properties (page[subdomain], page[domain_id]); investigate other Unbounce endpoints that modify domain bindings; check if other branded domain services (white-label platforms) have similar parameter injection flaws; examine API endpoints for similar mass assignment issues
...
## Notes
This is a bypass of a previous security fix, demonstrating the need for comprehensive remediation rather than point fixes. The attacker (ak1t4) provided detailed PoC including intercepted HTTP requests, video evidence, and step-by-step reproduction. The vulnerability affects all branded domains under Unbounce, making it a high-impact finding. The use of URL encoding (page%5Burl%5D = page[url]) bypasses some naive filtering. The vulnerability requires authentication but impacts high-value branded domains used by customers.
