# IDOR - Ability to view unlisted/draft products via API

## Metadata
- **Source:** HackerOne
- **Report:** 172545 | https://hackerone.com/reports/172545
...
## Summary
An attacker can access unpublished/draft product listings by directly calling the API endpoint /api/listings/{ID}/product_bundle with arbitrary listing IDs, bypassing the web UI's access controls. While the frontend prevents viewing unlisted products, the API lacks proper authorization checks, allowing any authenticated user to retrieve sensitive product details of draft listings belonging to other users.

## Attack scenario
1. Attacker creates an account and logs into the Reverb sandbox site
2. Attacker creates their own product listing to identify the API endpoint pattern (/api/listings/{ID}/product_bundle)
3. Attacker intercepts the API request using a proxy tool (Burp Suite)
4. Attacker modifies the listing ID in the API call from their own ID to a target ID of a known draft listing
5. Attacker sends the modified request and receives full product details (price, description, shop info, images, inventory) that should be hidden
6. Attacker can enumerate multiple listing IDs to gather competitive intelligence or sensitive product information from competitors

## Root cause
The API endpoint /api/listings/{ID}/product_bundle performs insufficient authorization validation. It likely checks only that the user is authenticated (not whether they are the listing owner) before returning product details. The web UI implements proper access control by redirecting to /marketplace, but this protection is not replicated at the API layer.

## Attacker mindset
An attacker would exploit this to gather competitive intelligence on draft products before launch, identify pricing strategies, discover unreleased products, or harass competitors by accessing their private listings. The ease of exploitation (simple ID enumeration) makes this attractive for low-effort reconnaissance.

## Defensive takeaways
- Implement server-side authorization checks on all API endpoints, not just the web UI. Verify the authenticated user owns the resource or has explicit permission to access it
- Apply the principle of least privilege: draft/unpublished resources should return 403 Forbidden or 404 Not Found unless the requester is the owner
- Use consistent access control logic across all layers (web UI, API, backend services). Never assume frontend checks are sufficient
- Conduct API-specific security testing during development; IDOR vulnerabilities commonly exist in APIs because developers assume web UI protections are sufficient
- Implement audit logging for API access to draft/sensitive resources to detect enumeration attacks
- Consider requiring additional authentication/authorization tokens for sensitive endpoints beyond basic session authentication

## Variant hunting
Look for similar IDOR issues in other API endpoints that return user-specific data: /api/listings/{ID}/details, /api/users/{ID}/inventory, /api/shops/{ID}/drafts, /api/orders/{ID}, or any endpoint that returns resources that should be access-restricted. Check if pagination/filtering endpoints on collections properly validate ownership.

## MITRE ATT&CK
- T1190 - Exploit Public-Facing Application (IDOR in API)
- T1526 - Enumerate External Targets (listing ID enumeration)
- T1526.004 - Enumerate External Targets: Cloud Instances (API reconnaissance)
- T1087 - Account Discovery (gathering user shop information)

## Notes
The report demonstrates good security research methodology: clear reproduction steps, proxy usage for API analysis, and explanation of the security impact. The example response shows the attacker also discovered stored XSS vectors in product fields (img onerror=alert(1)), suggesting multiple security issues in this feature. The contrast between frontend behavior (proper redirect) and API behavior (information disclosure) is a classic IDOR pattern that should trigger security review of all API endpoints.
