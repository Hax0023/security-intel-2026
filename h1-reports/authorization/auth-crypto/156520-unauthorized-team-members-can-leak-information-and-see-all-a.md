# Unauthorized Access to Admin Endpoints via Signature Token After Team Member Removal

## Metadata
- **Source:** HackerOne
- **Report:** 156520 | https://hackerone.com/reports/156520
- **Submitted:** 2016-08-04
- **Reporter:** eboda
- **Program:** Algolia
- **Bounty:** Not specified in writeup
- **Severity:** Critical
- **Vuln:** Broken Access Control, Improper Authentication, Information Disclosure, Privilege Escalation, Session Management Flaw
- **CVEs:** None
- **Category:** auth-crypto

## Summary
Removed or restricted team members could exploit a persistent signature token to access sensitive /1/admin/* endpoints despite having their access revoked. This allowed attackers to enumerate all indexes, view complete API call logs including sensitive data, and bypass dashboard access restrictions.

## Attack scenario
1. Attacker is invited as a team member with restrictive permissions (e.g., Search-only access to single index)
2. Attacker extracts the signature token from page source code when accessing the dashboard
3. Attacker is subsequently removed from the team or their permissions are restricted
4. Attacker crafts POST requests to /1/admin/listIndexes endpoint using stored signature and applicationID
5. Attacker crafts POST requests to /1/admin/userlogs endpoint to retrieve complete audit logs of all API calls including request/response bodies
6. Attacker gains visibility into sensitive search results, record modifications, and application activity across all indexes

## Root cause
The signature token used for admin endpoint authentication was not invalidated when team member access was revoked or restricted. The backend authentication mechanism relied solely on signature validation without checking current team member status or role permissions in real-time.

## Attacker mindset
An attacker with initial legitimate access (as invited team member) could gather the signature token before being removed, then use it indefinitely to bypass all access controls and observe sensitive operational data and user searches.

## Defensive takeaways
- Implement token expiration and rotation policies for all authentication tokens
- Validate current user permissions and team membership status on every admin endpoint request, not just signature verification
- Revoke all active sessions and tokens immediately when team member access is removed
- Use short-lived tokens for sensitive operations instead of long-lived signatures
- Implement proper role-based access control (RBAC) checks on backend endpoints independent of client-side tokens
- Log all access to admin endpoints and implement real-time alerting for access pattern anomalies
- Ensure audit logs themselves are access-controlled and not exposed through admin endpoints without authorization verification

## Variant hunting
Check if other API keys or tokens persist after user removal
Verify if signatures from different permission levels can access each other's data
Test if deleted team members can still use stored signatures across different applications
Examine if rate limiting exists on admin endpoints
Check if signature tokens work across different regions/data centers
Test if team member demotion properly revokes signature access
...
## Notes
This vulnerability demonstrates a critical flaw in Algolia's team access control system. The persistent nature of the signature token combined with lack of real-time authorization checks created a significant data exposure risk. The /1/admin/userlogs endpoint was particularly dangerous as it leaked complete request/response bodies including sensitive search results.
