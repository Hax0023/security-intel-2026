# RCE via unsafe inline Kramdown options when rendering Wiki pages

## Metadata
- **Source:** HackerOne
- **Report:** 1125425 | https://hackerone.com/reports/1125425
- **Submitted:** 2021-03-14
- **Reporter:** vakzz
- **Program:** GitLab
- **Bounty:** Not specified in writeup
- **Severity:** critical
- **Vuln:** Remote Code Execution, Unsafe Deserialization, Arbitrary File Inclusion, Object Instantiation via Reflection
- **CVEs:** None
- **Category:** memory-binary

## Summary
GitLab's wiki renderer processes markdown files with unsafe Kramdown inline options that allow arbitrary Ruby class instantiation. By chaining the Redis class with a malicious driver parameter, attackers can achieve RCE through arbitrary file inclusion and code execution via Ruby's require mechanism.

## Attack scenario
1. Attacker uploads a malicious Ruby file as a snippet attachment, obtaining its path within the uploads directory
2. Attacker creates a .rmd wiki file containing Kramdown inline options with a code block
3. Kramdown parses the inline options and extracts the syntax_highlighter_opts containing formatter and driver parameters
4. The formatter parameter is set to 'Redis' class, which gets instantiated via const_get validation bypass
5. The driver parameter contains a path traversal payload pointing to the attacker's uploaded Ruby file
6. Redis client initialization processes the driver option, using require to load the traversal path, executing the attacker's code

## Root cause
Multiple layers of insufficient validation: (1) Kramdown allows inline option overrides without sanitization, (2) Rouge formatter selection uses reflection without whitelisting, (3) Regex validation `/\A[[:upper:]][[:alnum:]_]*\z/` only blocks namespaced classes but allows any top-level class, (4) Redis driver parameter uses require without path validation, enabling directory traversal

## Attacker mindset
Exploit the trust in markdown rendering by chaining multiple permissive features (inline options → class instantiation → file inclusion) to bypass security boundaries. Leverage known classes already loaded in the application context rather than creating new attack surfaces.

## Defensive takeaways
- Disable or restrict Kramdown inline options processing in user-controlled content
- Implement strict whitelisting for formatter classes rather than blacklisting namespaces
- Sanitize all parameters passed to require/require_relative, prohibiting directory traversal sequences
- Use separate contexts or sandboxing for rendering untrusted markup
- Disable dangerous syntax highlighters or configure them with restricted class instantiation
- Validate and restrict file upload locations to prevent predictable path patterns
- Apply defense-in-depth: validate at markup parsing, class instantiation, and file inclusion layers
- Review all user-controllable parameters passed through markup rendering pipelines

## Variant hunting
Search for other classes with dangerous constructors loaded in Rails context (Gem::Installer, Gem::StubSpecification, Net::WriteAdapter). Investigate other markup renderers supporting inline options (reStructuredText, AsciiDoc). Test other syntax highlighters beyond Rouge. Examine snippet attachment functionality for alternative file placement mechanisms.
...
## Notes
This vulnerability exemplifies how seemingly independent features combine to create critical security issues. The regex validation was intended as a security measure but only blocked double-colon namespaces, allowing single-level class access. The reliance on require for dynamic loading without input validation is a classic RCE vector. The vulnerability chain required knowledge of GitLab's infrastructure (default upload path) and loaded gem internals (Redis driver mechanism).
