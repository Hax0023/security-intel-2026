# Remote Code Execution via GitHub Import and Redis Command Injection

## Metadata
- **Source:** HackerOne
- **Report:** 1672388 | https://hackerone.com/reports/1672388
- **Submitted:** 2022-08-17
- **Reporter:** yvvdwf
- **Program:** GitLab
- **Bounty:** Not specified in report
- **Severity:** Critical
- **Vuln:** Unsafe Deserialization, Redis Command Injection, Object Injection, Improper Input Validation
- **CVEs:** None
- **Category:** memory-binary

## Summary
GitLab's GitHub import functionality uses Octokit with Sawyer::Resource, which converts API response hashes into objects with dynamic methods. An attacker can craft malicious GitHub import responses containing nested objects with attributes like 'bytesize' and 'to_s' to inject arbitrary Redis commands, ultimately achieving RCE through deserialization gadgets or Redis replication attacks.

## Attack scenario
1. Attacker creates a malicious GitHub repository or controls a GitHub API response endpoint
2. Attacker crafts a response containing Sawyer::Resource with nested hash containing 'to_s' and 'bytesize' keys
3. GitLab imports the GitHub data and passes the Sawyer object to caching functions expecting numeric IDs
4. The injected 'bytesize' attribute truncates Redis commands, allowing injection of arbitrary Redis commands into the command pipeline
5. Attacker executes Redis commands such as REPLICAOF to gain control of GitLab's Redis instance or injects deserialization payloads
6. RCE achieved through Redis gadget chains (e.g., Gem::Requirement, Marshal.dump) or by poisoning cached objects with malicious Ruby code

## Root cause
GitLab directly uses untrusted Sawyer::Resource objects returned from GitHub API in cache operations without proper type validation or sanitization. Sawyer dynamically creates object methods from hash keys, and when these objects are used as function arguments (like cache IDs), their 'to_s' and other methods can be overridden with malicious values that inject Redis protocol commands.

## Attacker mindset
An attacker recognizes that GitLab treats third-party API responses as trusted objects and exploits the dynamic method resolution in Sawyer to construct valid Redis commands with injected payloads. By understanding Redis protocol encoding and Ruby deserialization gadgets, they escalate from SSRF/injection to full RCE.

## Defensive takeaways
- Never trust third-party library objects (like Sawyer::Resource) in security-critical operations; validate and extract only expected primitive types
- Sanitize all inputs before constructing Redis commands, especially when user-controlled data flows through dynamic object serialization
- Implement strict type checking on all function parameters, particularly for cache keys and IDs that should be integers or strings
- Disable dangerous Redis commands (beyond just 'keys') in production environments, including REPLICAOF, SCRIPT LOAD, and EVAL
- Use parameterized/prepared Redis command interfaces instead of string concatenation to prevent command injection
- Regularly audit and restrict Ruby deserialization (Marshal.load) of untrusted data; use safer alternatives like JSON
- Implement network segmentation between application and Redis; restrict Redis connections to localhost only

## Variant hunting
Similar vulnerabilities could exist in other GitLab integrations accepting third-party API responses (GitBucket, Gitea, Bitbucket). Any codebase using Sawyer or similar dynamic object mapping libraries with untrusted API data is at risk. Look for patterns where API responses are passed directly to cache/storage functions without type validation.
...
## Notes
Reporter demonstrated proof-of-concept with REPLICAOF command injection against gitlab.com and attempted Marshal.dump deserialization payloads. The vulnerability chains multiple weaknesses: (1) dynamic object method resolution in Sawyer, (2) lack of input validation in cache operations, and (3) dangerous Redis command availability. Reporter demonstrated responsible disclosure by ceasing testing after confirming impact.
