# CSRF-tokens cached by CloudFlare proxy via Web Cache Deception, enabling Account Takeover

## Metadata
- **Source:** HackerOne
- **Report:** 260697 | https://hackerone.com/reports/260697
- **Submitted:** 2017-08-16
- **Reporter:** fransrosen
- **Program:** Discourse
- **Bounty:** Not specified in report
- **Severity:** Critical
- **Vuln:** Web Cache Deception, CSRF Token Exposure, Improper Cache-Control Headers, Account Takeover
- **CVEs:** None
- **Category:** web-api

## Summary
Discourse instances behind CloudFlare proxy fail to set no-cache headers on routes exposing CSRF tokens and usernames, allowing attackers to exploit Web Cache Deception to retrieve cached CSRF tokens from the same CloudFlare region and perform account takeover via email change. The vulnerability affects multiple routes including /u/x.css and /u/my/preferences.css which return 200 status despite file extension tricks.

## Attack scenario
1. Attacker identifies a Discourse instance behind CloudFlare proxy without proper cache headers
2. Attacker crafts malicious webpage that lures victim to visit while authenticated to target Discourse
3. Malicious page issues requests to /u/$random.css endpoints to poison CloudFlare cache with victim's CSRF token and username header
4. Attacker's server (located in same CloudFlare region as victim) fetches the cached .css URL to retrieve victim's CSRF token from HTML meta tag and X-Discourse-Username header
5. Attacker uses extracted CSRF token and username to craft POST request changing victim's email address
6. Victim receives account takeover confirmation; attacker completes email verification to gain full account control

## Root cause
Discourse routes serving authenticated user data (CSRF tokens, usernames) lack Cache-Control, Pragma, or Expires headers to prevent caching. CloudFlare treats URLs with cacheable file extensions (.css) as static content regardless of response headers or status codes, allowing poisoning of regional cache with sensitive tokens.

## Attacker mindset
Sophisticated threat actor with knowledge of CDN caching mechanics and regional infrastructure. Attack requires victims to visit attacker-controlled page while authenticated, and attacker infrastructure positioned in same CloudFlare region. Monetizable via account takeover for credential theft, social engineering, or account sale.

## Defensive takeaways
- Add explicit Cache-Control: no-cache, no-store, must-revalidate headers to all authenticated routes returning user-specific data
- Implement Pragma: no-cache headers on sensitive endpoints as defense-in-depth
- Strip or prevent CSRF token inclusion in error pages and 404 responses
- Validate and reject requests with suspicious file extensions on authenticated routes
- Implement CloudFlare configuration to disable caching for HTML content or specific paths
- Use Set-Cookie with Secure, HttpOnly, SameSite=Strict flags to limit token exposure vectors
- Consider double-submit CSRF pattern or origin validation to mitigate token compromise
- Regular security audits of all routes exposing user identifiers or sensitive headers

## Variant hunting
Check other Django/Rails frameworks for similar cache-header omissions on authenticated routes
Test all user preference and account pages with various file extensions (.css, .js, .woff, .svg, .gif)
Audit error pages (404, 500) for CSRF token or user data exposure
Investigate Nginx/Apache default cache behaviors when behind proxies without explicit headers
Test other CDN providers (Cloudflare alternatives) for similar cache deception vectors
Search for routes returning X-Discourse-Username or similar user-identifying headers without cache controls
Examine whether CSRF tokens appear in redirects or Location headers
...
## Notes
This is a critical chaining of Web Cache Deception (infrastructure misconfiguration) with CSRF token exposure (application flaw). The attack requires precise regional positioning but affects broad victim population within CloudFlare regions. Key insight: file extension appending bypasses normal authentication checks due to Discourse routing, while CloudFlare obeys mime-type rules independently of authentication. PoC attached in original report demonstrates full email change workflow. Affects all Discourse instances using CloudFlare without explicit cache headers, making this a widespread vulnerability in a popular platform.
...
Add the `no-cache` headers, `Cache-Control` and/or `Expire` on any of the templates that outputs t
